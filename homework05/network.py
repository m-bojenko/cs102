...
def get_network(users, as_edgelist=True):
    vertices = [user.id for user in users]  # создаём вершины с id
    vertices_names = [user.first_name + ' ' + user.last_name for user in users]  # создаём вершины с ИФ
    edges_map = [[0 for col in range(len(vertices))]  # матрица смежности (as_edgelist=False)
                 for row in range(len(vertices))]
    edges = []  # список рёбер (...=True)
    for user in users:
        friends = get_friends(user.id, 'bdate')
        if friends:
            for friend in friends:
                try:
                    vertices.index(friend.id)  # ищём общих друзей
                except:
                    pass
                else:
                    if as_edgelist:
...
def plot_graph(graph):
    ...
    igraph.plot(g, 'pic0.png', **visual_style)


our_id = int(input('Enter id: '))
us_fr = get_friends(our_id, 'bdate')
our_network = get_network(us_fr, as_edgelist=True)
gr = plot_graph(our_network)
